 ...
 * @private
 */
export function* callAPI_(url: string, opts?: Object): Generator<Effect, CallAPIResult, *> {
  try {
    const response: Response = yield call(fetchJsonp, url, {
      ...defaultCallAPIOptions,
      ...opts
    });

    const body: string = yield apply(response, response.json);
    return {
      status: 200,
      body
    };
 ...
 * @returns {*} A saga effect
 */
export default function* callAPI(url: string, opts?: Object): Generator<Effect, CallAPIResult, *> {
